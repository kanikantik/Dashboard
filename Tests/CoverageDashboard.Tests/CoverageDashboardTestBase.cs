...
using CoverageDashboard.Migrations.SeedData;
...
{
    public abstract class CoverageDashboardTestBase : AbpIntegratedTestBase<CoverageDashboardTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected CoverageDashboardTestBase()
        {
        ...
        protected void UsingDbContext(Action<CoverageDashboardDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<CoverageDashboardDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CoverageDashboardDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CoverageDashboardDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CoverageDashboardDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CoverageDashboardDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<CoverageDashboardDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CoverageDashboardDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CoverageDashboardDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CoverageDashboardDbContext, Task<T>> func)
        {
            T result;
